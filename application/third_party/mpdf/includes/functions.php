...
if(!function_exists('strcode2utf')){ 
  function strcode2utf($str,$lo=true) {
	...
	$str = preg_replace_callback( '/\&\#([0-9]+)\;/', create_function( '$matches', 'return code2utf($matches[1], $lo);' ), $str );
	$str = preg_replace_callback( ' / \&\ #x([0-9a-fA-F]+)\;/', create_function( '$matches', 'return codeHex2utf($matches[1], $lo);' ), $str );
	return $str;
  }
}
