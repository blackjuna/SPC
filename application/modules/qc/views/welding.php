    ...
    <section class="content">
      ...
      <div class="box box-primary">
        ...
        <div class="box-footer">
          <button class="btn btn-info pull-right" onclick="add_welding()"><i class="glyphicon glyphicon-search"></i> SEARCH</button>
        </div>
        <!-- /.box-footer-->
      </div>
      <!-- /.box -->
      ...
      <div class="box">
        ...
        <!-- /.box-body -->
        <div class="box-footer">
          <button class="btn btn-success" onclick="add_welding()"><i class="glyphicon glyphicon-plus"></i> Add</button>
...
$(document).ready(function() {
  ...
  table = $('#tables').DataTable({ 
      ...
      "ajax": {
          "url": "<?php echo site_url('qc/welding/r')?>",
          "type": "POST"
      },
      "columns": [
  ...
  $('#tables tbody').on( 'click', 'tr', function () {
      ...
      id_welding = table.row( this ).data().id;
      // blanking_time(id_blanking);
      // blanking_data(null);
      welding_all(id_welding);
  } );
...
function welding_all(id)
{
  $.ajax({
    "url": "<?php echo site_url('qc/welding_all')?>/"+id,
    "dataType": "json",
    // "bDestroy":"true",
    // "retrieve": true,
  ...
  $.ajax({
    "url": "<?php echo site_url('qc/welding_all_dt')?>/"+id,
    "dataType": "json",
    // "bDestroy":"true",
    // "retrieve": true,
    "success": function(json) {
...
function add_welding()
{
    ...
    $('.modal-title').text('Add welding Dimensi'); // Set Title to Bootstrap modal title
    $(".dataa").remove();
    $(".datalabel").remove();
}

function edit_welding()
{
  table = $('#tables4').DataTable({ 
      ...
      // Load data for the table's content from an Ajax source
      "ajax": {
          "url": "<?php echo site_url('qc/wiwelding/r')?>",
          "type": "POST"
      },
      "columns": [
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('qc/welding/c')?>";
    } else {
        url = "<?php echo site_url('qc/welding/u')?>";
    }
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Welding Form</h3>
            </div>
