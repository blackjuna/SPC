    ...
    <section class="content">
      ...
      <div class="box box-primary">
        ...
        <div class="box-footer">
          <button class="btn btn-info pull-right" onclick="add_winding_pressure()"><i class="glyphicon glyphicon-search"></i> SEARCH</button>
        </div>
        <!-- /.box-footer-->
      </div>
      <!-- /.box -->
      ...
      <div class="box">
        ...
        <!-- /.box-body -->
        <div class="box-footer">
          <button class="btn btn-success" onclick="add_winding_pressure()"><i class="glyphicon glyphicon-plus"></i> Add</button>
...
$(document).ready(function() {
  ...
  table = $('#tables').DataTable({ 
      ...
      "ajax": {
          "url": "<?php echo site_url('qc/winding_pressure/r')?>",
          "type": "POST"
      },
      "columns": [
  ...
  $('#tables tbody').on( 'click', 'tr', function () {
      ...
      id_winding_pressure = table.row( this ).data().id;
      // blanking_time(id_blanking);
      // blanking_data(null);
      winding_pressure_all(id_winding_pressure);
  } );
...
function winding_pressure_all(id)
{
  $.ajax({
    "url": "<?php echo site_url('qc/winding_pressure_all')?>/"+id,
    "dataType": "json",
    // "bDestroy":"true",
    // "retrieve": true,
  ...
  $.ajax({
    "url": "<?php echo site_url('qc/winding_pressure_all_dt')?>/"+id,
    "dataType": "json",
    // "bDestroy":"true",
    // "retrieve": true,
    "success": function(json) {
...
function add_winding_pressure()
{
    ...
    $('.modal-title').text('Add Winding Pressure'); // Set Title to Bootstrap modal title
    $(".dataa").remove();
    $(".datalabel").remove();
}

function edit_winding_pressure()
{
  ...
  // $('.form-group').removeClass('has-error'); // clear error class
  // $('.help-block').empty(); // clear error string
  $('#edit_winding_pressure_form').modal('show'); // show bootstrap modal
  $('.modal-title').text('Edit welding'); // Set Title to Bootstrap modal title
}
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('qc/winding_pressure/c')?>";
    } else {
        url = "<?php echo site_url('qc/winding_pressure/u')?>";
    }
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Winding Pressure Form</h3>
            </div>
