...
if ( ! function_exists('is_allow'))
{
	function is_allow($crud=NULL, $mdl_grp=NULL, $mdl=NULL) {
		...
		$qry = $ci->db->get_where( 'c_modules_group', array('code'=>$mdl_grp) );
		if ( $qry->num_rows() < 1 ) {
			$ci->db->insert( 'c_modules_group', array('code'=>strtoupper($mdl_grp), 'name'=>strtoupper($mdl_grp), 'active'=>1));
		...
		$qry = $ci->db->get_where( 'c_modules', array('code'=>$mdl, 'modules_group_id'=>$module_group_id) );
		if ( $qry->num_rows() < 1 ) {
			$ci->db->insert('c_modules', array('modules_group_id'=>$module_group_id, 'code'=>strtoupper($mdl), 'name'=>strtoupper($mdl), 'active'=>1));
			$module_id = $ci->db->insert_id();
		} else {
			$module_id = $qry->row()->id;
		}
		
		$query = $ci->db->query("select group_id from c_users_groups where user_id = $user_id");
		if ($query->num_rows() > 0) {
			foreach ($query->result() as $row) {
				$query2 = $ci->db->query("select * from c_groups_auth where groups_id = $row->group_id and modules_id = $module_id and $crud = 1");
